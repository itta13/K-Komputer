...
class KNN
{
	...
	function dataset_nilai()
	{
		...
		foreach ($this->dataset as $key => $val) {
			foreach ($val as $k => $v) {
				...
				$this->dataset_nilai[$key][$k] = ($k == $this->target || !isset($ATRIBUT_NILAI[$k])) ? $v :  $this->nilai_konversi[$v];
			}
		}
	}
	...
	function nearest()
	{
		...
		foreach ($this->jarak as $key => $val) {
			$this->nearest[] = $key;
			if (!isset($this->total[$this->normal[$key][$this->target]]))
				$this->total[$this->normal[$key][$this->target]] = 0;
	...
	function jarak()
	{
		...
		foreach ($this->nilai as $k => $v) {
			$v = isset($this->nilai_konversi[$v]) ? $this->nilai_konversi[$v] : $v;
